...
{
	return os << delimeter << "Номер: " << obj.first << "\tПреступление: " << obj.second;
}
int menu();
...
std::map<std::string, std::list<Crime>>& add(std::map<std::string, std::list<Crime>>& database)
{
	...
	cin >> violation_id;
	std::string place;
	cout << "Введите где было совершено преступление: "; std::getline(cin, place);
	std::string time;
	cout << "Введите когда было совершено преступление: "; std::getline(cin, place);
